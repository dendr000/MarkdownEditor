// src/components/diagram/DiagramModal.jsx v10.0
/*
 * 파일 설명: 분리된 유틸리티(diagramParser)를 사용하여 파싱 로직을 외주화하고, UI 관리만 담당하는 경량화된 다이어그램 모달 컴포넌트입니다.
 */
...
function DiagramModal({ isOpen, onClose, onInsert, initialDiagramMarkdown = '' }) {
  ...
  const activeCode = editMode === 'gui' ? buildStateToMarkdown(diagramType, { pieTitle, pieItems, flowOrientation, flowSteps, seqParticipants, seqMessages }) : rawCode;
        ...
        <div className="diagram-modal-body">
          ...
          <div className="diagram-preview-panel">
            <div className="preview-render-wrapper">
              {diagramType.startsWith('mermaid') && <MermaidBlock chart={activeCode} onNodeClick={setActiveNodeId} />}
              {diagramType === 'geojson' && <GeoJsonBlock dataString={activeCode} />}
              {diagramType === 'stl' && <StlBlock stlString={activeCode} />}
            </div>
          </div>
        </div>
