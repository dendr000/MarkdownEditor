// src/components/table/html-table/ToolbarStyle.jsx v1.4
/*
 * 파일 위치: src/components/table/html-table/ToolbarStyle.jsx
 * 기능 요약: HTML 표 내부의 텍스트 서식(굵게, 기울임, 취소선), 정렬, 글자색 및 배경색을 제어하는 스타일 툴바 컴포넌트입니다. (배포를 위해 콘솔 로그 출력 기능이 제거되었습니다.)
      ...
      <div className="toolbar-row">
        <div className="toolbar-left">
          <div className="control-group">
            ...
            <button onClick={() => { handleAlignChange('left'); }} disabled={!hasFocus} className={activeCell?.align === 'left' ? 'active' : ''} title="왼쪽 정렬">
              <AlignLeft size={16} />
            </button>
            <button onClick={() => { handleAlignChange('center'); }} disabled={!hasFocus} className={activeCell?.align === 'center' ? 'active' : ''} title="가운데 정렬">
              <AlignCenter size={16} />
            </button>
            <button onClick={() => { handleAlignChange('right'); }} disabled={!hasFocus} className={activeCell?.align === 'right' ? 'active' : ''} title="오른쪽 정렬">
              <AlignRight size={16} />
            </button>
          </div>
          ...
          <div className="control-group">
            ...
            <button onClick={() => { clearFormatting(); }} disabled={!hasFocus} title="모든 서식 지우기">
              <Eraser size={16} />
            </button>
          </div>
        </div>
        ...
        <div className="toolbar-right">
          <div className="control-group">
            ...
            <select
              ...
              onChange={(e) => { applyColor('bgColor', e.target.value); }}
              disabled={!hasFocus}
            >
